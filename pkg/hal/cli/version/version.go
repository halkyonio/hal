...
func NewCmdVersion(parentCmdName string) *cobra.Command {
	...
	version := &cobra.Command{
		...
		Args:    cobra.NoArgs,
		Example: cmdutil.CommandName(commandName, parentCmdName),
...
func Version() string {
	return fmt.Sprintf("%s %s built with ❤️ by the Halkyon team on '%s' (commit: %s) running on top of '%s'", parent, version, date, commit, k8s.GetK8SClientFlavor())
}
