...
func main() {
	...
	var clidoc = &cobra.Command{
		...
		Example: `  # Generate a markdown-formatted CLI reference page for hal reference > cli-reference.adoc

  # Generate the CLI reference
  hal.go reference > cli-reference.ado`,
		...
		Run: func(command *cobra.Command, args []string) {
			if len(args) == 0 {
				...
				switch args[0] {
				case "reference":
					referencePrinter(cli.NewCmdHal(), 0)
				default:
					fmt.Print(command.Usage())
				}
			}
